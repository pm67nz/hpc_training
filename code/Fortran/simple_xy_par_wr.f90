     ...
     !     sample
     !     data. To handle this in netCDF we create two shared
     !     dimensions,
     !     "x" and "y", and a netCDF variable, called "data". It uses
     !     parallel I/O to write the file from all processors at the
     !     same
     !     time.
     ...
     program simple_xy_par_wr
       ...
       ! When we create netCDF files, variables and dimensions, we get
       ! back
       ! an ID for each one.
       ...
       ! MPI stuff: number of processors, rank of this processor, and
       ! error
       ! code.
       ...
       ! Initialize MPI, learn local rank and total number of
       ! processors.
       call MPI_Init(ierr)
       ...
       ! Write the pretend data to the file. Each processor writes one
       ! row.
       start = (/ 1, my_rank + 1/)
       count = (/ p, 1 /)
